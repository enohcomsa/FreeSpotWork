...
import { BuildingLegacy, FloorLegacy, RoomLegacy, SubjectItem, TimeTableItem } from '@free-spot/models';
...
export class AdminRoomDetailComponent implements OnInit {
  ...
  onSubjectListChange(changedSubjectList: SubjectItem[]): void {
    ...
    const updatedRoom: UpdateRoomCmd = {
      subjectList: changedSubjectList.map((cs)=>cs.name),//should be ObjectId
  ...
  //     ...this.floorSig(),
  //     roomList: this.floorSig().roomList.map((room: RoomLegacy) => (room.name === updatedRoom.name ? updatedRoom : room)),
  //   };
