export { BookedEvent } from './lib/booked-event.model';
export { BuildingLegacy } from './lib/building.model';
export { DynamicSerach } from './lib/dynamic-search.model';
export { Faculty } from './lib/faculty.model';
export { FloorLegacy } from './lib/floor.model';
export { FreeSpotDate } from './lib/free-spot-app-date.model';
export { FreeSpotUser } from './lib/free-spot-user.model';
export { Group } from './lib/group.model';
export { RoomLegacy } from './lib/room.model';
export { SemiGroup } from './lib/semi-group.model';
export { SubjectItemLegacy } from './lib/subject.model';
export { TimetableActivityItemLegacy } from './lib/timetable-activity-item.model';
export { TimetableDayItemLecagy, TimetableDayItem } from './lib/timetable-day-item.model';
export { TimeTableItemLecagy } from './lib/timetable-item.model';
export { Year } from './lib/year.model';
