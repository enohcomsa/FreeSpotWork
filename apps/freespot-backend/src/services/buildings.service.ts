import type { BuildingCreateRequest, BuildingUpdateRequest, BuildingResponseDto } from "../schemas/buildings.zod";
import * as repo from "../repos/buildings.repo";
import { NotFoundError } from "./errors";
import { mapMongoError } from "./mongo";

export async function getBuildings(): Promise<BuildingResponseDto[]> { return repo.listBuildings(); }
...
export async function createBuilding(input: BuildingCreateRequest): Promise<BuildingResponseDto> {
  try { return await repo.createBuilding(input); } catch (e) { mapMongoError(e); }
}
